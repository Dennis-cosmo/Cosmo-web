...
export async function GET() {
  try {
    console.log("Obteniendo actividades de taxonomía desde API externa");
    ...
    const processedActivities = activitiesResponse.data.map((item: any) => {
      ...
      const activityData = item.activity || item; // Puede venir directo o anidado
    ...
    console.log(`Actividades obtenidas: ${processedActivities.length}`);
